PROC scx_cpu1_hk_sendoutmsg(OutputPktID, DataBytePattern[], PktNo, Checking, StaleData)
;*******************************************************************************
...
wait 5
currentCnt = SCX_CPU1_HK_CMBPKTSSENT
count =  SCX_CPU1_HK_CMBPKTSSENT+1
missing = SCX_CPU1_HK_MISSDATACTR+1

ut_setupevents "SCX","CPU1",{HKAppName},HK_OUTPKT_MISSING_DATA_EID,"DEBUG", 1
...
for entry=1 to 80 do 
    if (PktNo = 1) then
      Data[entry-1] = scx_cpu1_HK_Tab1Pkt1Data[entry] 
    elseif (PktNo = 2) then
      Data[entry-1] = scx_cpu1_HK_Tab1Pkt2Data[entry] 
    elseif (PktNo = 3) then
      Data[entry-1] = scx_cpu1_HK_Tab1Pkt3Data[entry] 
    elseif (PktNo = 4) then
      Data[entry-1] = scx_cpu1_HK_Tab1Pkt4Data[entry] 
    elseif (PktNo = 5) then
      Data[entry-1] = scx_cpu1_HK_Tab1Pkt5Data[entry] 
    elseif (PktNo = 6) then
      Data[entry-1] = scx_cpu1_HK_Tab1Pkt6Data[entry] 
    else
...
if (count = scx_cpu1_HK_CMBPKTSSENT) then
...
if (Checking = MissingYes) then
  wait 5
  if (missing =  scx_cpu1_HK_MISSDATACTR) then
  ...
  if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
    write "<*> Passed (2001.3) - Event message ",SCX_CPU1_find_event[1].eventid, " received"
    ut_setrequirements HK_20013, "P"
  else
    write "<!> Failed (2001.3) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",HK_OUTPKT_MISSING_DATA_EID, "."
    ut_setrequirements HK_20013, "F"
  endif
...
write ";*********************************************************************"
write ";  End procedure SCX_CPU1_hk_sendoutmsg                        "
write ";*********************************************************************"
ENDPROC
