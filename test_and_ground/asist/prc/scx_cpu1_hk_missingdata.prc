...
s SCX_CPU1_hk_sendoutmsg(OutputPacket1, DataBytePattern, Pkt1, MissingYes, 40)
...
s SCX_CPU1_hk_sendoutmsg(OutputPacket2, DataBytePattern, Pkt2, MissingYes, 18)
...
s SCX_CPU1_hk_sendoutmsg(OutputPacket3, DataBytePattern, Pkt3, MissingYes, 1)
...
ut_tlmwait SCX_CPU1_find_event[1].num_found_messages, 14
if (UT_TW_Status = UT_Success) then
  write "<*> Passed (2001.5) - Rcv'd the expected error events for short packets"
  ut_setrequirements HK_20015, "P"
else
  write "<!> Failed (2001.5) - Did not rcv the expected error events for short packets. Expected 14; Got ",SCX_CPU1_find_event[1].num_found_messages
  ut_setrequirements HK_20015, "F"
endif

if (HK_DISCARD_INCOMPLETE_COMBO = 1) then
  local currentCnt = SCX_CPU1_HK_CMBPKTSSENT
  s SCX_CPU1_hk_sendoutmsg(OutputPacket1, DataBytePattern, Pkt1, MissingYes, 40)

  if (currentCnt = SCX_CPU1_HK_CMBPKTSSENT) then
...
ut_sendcmd "SCX_CPU1_TBL_VALIDATE INACTIVE VTABLENAME=HKCopyTblName"
wait 20

ut_sendcmd "SCX_CPU1_TBL_ACTIVATE ATABLENAME=HKCopyTblName"
wait 10
...
s SCX_CPU1_hk_sendoutmsg(OutputPacket2, DataBytePattern, Pkt2, MissingYes, 1)
...
write ";*********************************************************************"
write ";  End procedure SCX_CPU1_hk_missingdata                        "
write ";*********************************************************************"
ENDPROC
